...
//                  [modelOut, stat] = lolimot_learn(data, sigma, nbpart, maximp, nbCut, vec, Log, modelIn, pLinear)
                    [modelOut, stat] = lolimot_learn(data, sigma, nbpart, maximp, nbCut, vec, Log)
                    Xp(:,j)   = lolimot_estim(absoraj, modelOut)
                    Xpval(:,j)= lolimot_estim(absorval, modelOut)
                else
...
//                  [modelOut, stat] = lolimot_learn(data, sigma, nbpart, maximp, nbCut, vec, Log, modelIn, pLinear)
                    ...
                    for aux=[1:size(absortest,'r')]
                        Xpt(i0-1+aux:j)= lolimot_estim(absortest(aux,:), modelOut)
                    end
